...
vertex VertexOut advanced_vertex(
                                 const device VertexIn* vertex_array [[ buffer(0) ]],const device Uniforms&  uniforms   [[ buffer(1) ]],
    ...
    VertexOut.contrast = uniforms.contrast;
...
fragment float4 textured_fragment(VertexOut interpolated [[stage_in]],
    ...
    float4 color = .5+interpolated.contrast*(tex2D.sample(sampler2D, interpolated.texCoord)-.5);
    return color;
}

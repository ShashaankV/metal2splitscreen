...
//  metal2splitscreen4
//
//  Created by svattiku on 12/4/18.
...
class ViewController: UIViewController {
    ...
    var contrast1: Float = 1.0
    var contrast2: Float = 0.1
    var contrast1Buffer: MTLBuffer!
    var contrast2Buffer: MTLBuffer!
    ...
    override func viewDidLoad() {
        ...
        q1 = Quad(device: device) // declare geometric object to render to
        
        //setup constrast variables
        contrast1Buffer = device!.makeBuffer( length: MemoryLayout.size(ofValue: contrast1), options: [])
        let bufferPointer1 = contrast1Buffer.contents()
        memcpy(bufferPointer1, &contrast1, MemoryLayout.size(ofValue: contrast1))
        
        contrast2Buffer = device!.makeBuffer( length: MemoryLayout.size(ofValue: contrast2), options: [])
        let bufferPointer2 = contrast2Buffer.contents()
        memcpy(bufferPointer2, &contrast2, MemoryLayout.size(ofValue: contrast2))
    ...
    func render() {
        ...
        renderEncoder?.setVertexBuffer(contrast1Buffer, offset: 0, index: 1) // send contrast parameter, define contrast1 above
        renderEncoder?.drawPrimitives(type: .triangle, vertexStart: 0, vertexCount: 6, instanceCount: 1) //draw command based on inputs above
        ...
        renderEncoder?.setRenderPipelineState(pipelineState)
        renderEncoder?.setVertexBuffer(q1.vertexBuffer, offset: 0, index: 0)
          renderEncoder?.setVertexBuffer(contrast2Buffer, offset: 0, index: 1) // send contrast parameter, define contrast1 above
